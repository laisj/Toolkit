  ...
  def main(args: Array[String]) {
    ...
    for (file <- fileArr) {
      ...
      val ideaData = parseData.map{ x=>
        ...
        val exp = x(16)
        val slot = x(10)
        val idea = (parse(x(15)) \ "ideaId").extract[String]
        val ad = (parse(x(15)) \ "adId").extract[String]
    ...
    for (file <- fileArr) {
      ...
      val ideaDataRaw = parseData.map{x=>
        ...
        val slot = x(10)
        val idea = (parse(x(15)) \ "ideaId").extract[String]
        val os = (parse(x(15)) \ "os").extract[Double]
        val sex = (parse(x(15)) \ "sex").extract[Double]
        val wifiType = (parse(x(15)) \ "wifiType").extract[Double]
        val label = x(1)
        val prediction = x(9)
    ...
    sc.parallelize(topAds, 1).saveAsTextFile("training_pipeline/debug/"+suffix+"/topAds")
    sc.parallelize(topAdsCpc, 1).saveAsTextFile("training_pipeline/debug/"+suffix+"/topAdsCpc")
    sc.parallelize(adsDebugBuf, 1).saveAsTextFile("training_pipeline/debug/"+suffix+"/adsDebugBuf")
