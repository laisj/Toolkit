...
val dt_yesterday = new Date(System.currentTimeMillis()-24*60*60*4000)
...
for (file <- fileArr) {
  ...
  val dimData2 = parseData.map{ x=>
    ...
    val posDisplay = (parse(x.split("\\|")(15)) \ "posDisplay2").extract[Double]
    val posClick = (parse(x.split("\\|")(15)) \ "posClick2").extract[Double]
    val userDisplay = (parse(x.split("\\|")(15)) \ "userDisplay").extract[Double]
    val userClick = (parse(x.split("\\|")(15)) \ "userClick").extract[Double]
    val label = x.split("\\|")(1)
...
sc.parallelize(topDimCpc).saveAsTextFile("training_pipeline/dimDebug/"+suffix+"/topAdsCpc")
